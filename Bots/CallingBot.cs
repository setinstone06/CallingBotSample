        ...
        protected override async Task OnMessageActivityAsync(ITurnContext<IMessageActivity> turnContext, CancellationToken cancellationToken)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        var meetingUrl = turnContext.Activity.Text.Substring("join ".Length);

                        Console.WriteLine(meetingUrl);

                        var statefulCall = await graph.JoinScheduledMeeting(meetingUrl);
                        Console.WriteLine(statefulCall);
                        await SendReponse(turnContext, $"Joined call, call id {statefulCall.Id.ToString()}", cancellationToken);
                    }
                    if (text.StartsWith("play"))
                    {
                        var split = turnContext.Activity.Text.Split(' ');
                        var callId = split[1];
                        var mediaUri = split[2];
                        await graph.PlayPrompt(callId, mediaUri);
                        await SendReponse(turnContext, $"Play media success", cancellationToken);
                    }
                }
                catch (Exception ex)
                {
                    await SendReponse(turnContext, $"Some error occured {ex.ToString()}", cancellationToken);
                }
            }
        }
        ...
        private async Task SendReponse(ITurnContext<IMessageActivity> turnContext, string input, CancellationToken cancellationToken)
        {
            switch (input)
            {
                ...
                case "inviteparticipant":
                    ...
                    break;
                default:
                    await turnContext.SendActivityAsync(input);
                    break;
            }
        }
